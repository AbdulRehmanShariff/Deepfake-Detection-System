...
def predict_audio(file_path):
    ...
    if audio_model is None:
        try:
            audio_model = keras.models.load_model(AUDIO_MODEL_PATH, compile=False)  # FIX
    ...
    try:
        ...
        if mfcc is None:
            return {"result": "Prediction failed", "confidence": 0.0, "reason": "Audio corrupt or unsupported."}

        from keras.utils import pad_sequences  # FIX
        padded = pad_sequences([mfcc], maxlen=MAX_PAD_LEN, padding='post', truncating='post', dtype='float32')[0]
        model_input = np.expand_dims(padded, axis=0)
        score = float(audio_model.predict(model_input)[0][0])
        label = "DEEPFAKE" if score < 0.5 else "REAL"
        return {"result": label, "confidence": round(score, 4)}
    except Exception as e:
        return {"result": "Prediction failed", "confidence": 0.0, "reason": str(e)}